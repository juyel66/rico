...
const API_BASE =
  (import.meta && (import.meta as any).env?.VITE_API_BASE) || "http://10.10.13.60:8000/api";
...
const pluralize = (count: number | string | undefined | null, singular: string, plural?: string) => {
    ...
    >
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} className="text-gray-500 text-3xl leading-none">
            &times;
          </button>
        </div>
...
const ShareModal: React.FC<ShareModalProps> = ({
  ...
  const platforms = [
    { name: "Facebook", icon: <FaFacebookF />, url: `https://www.facebook.com/sharer/sharer.php?u=${encoded}` },
    { name: "WhatsApp", icon: <FaWhatsapp />, url: `https://wa.me/?text=${encoded}` },
    { name: "Twitter", icon: <FaTwitter />, url: `https://twitter.com/intent/tweet?url=${encoded}&text=${encodedTitle}` },
    { name: "LinkedIn", icon: <FaLinkedinIn />, url: `https://www.linkedin.com/shareArticle?mini=true&url=${encoded}&title=${encodedTitle}` },
    { name: "Telegram", icon: <FaTelegramPlane />, url: `https://t.me/share/url?url=${encoded}&text=${encodedTitle}` },
    { name: "Pinterest", icon: <FaPinterestP />, url: `https://pinterest.com/pin/create/button/?url=${encoded}&description=${encodedTitle}` },
    { name: "Reddit", icon: <FaRedditAlien />, url: `https://reddit.com/submit?url=${encoded}&title=${encodedTitle}` },
    { name: "Email", icon: <FaEnvelope />, url: `mailto:?subject=${encodedTitle}&body=${encoded}` },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 m-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-4 mb-3">
          <img src={previewImageUrl} alt="preview" className="w-28 h-20 object-cover rounded-md border" />
          <div className="flex-1">
            <div className="font-semibold text-gray-800">{propertyTitle}</div>
            <div className="text-sm text-gray-500 truncate">{propertyUrl}</div>
          </div>
          <button onClick={copyLink} className="bg-gray-100 border px-3 py-2 rounded"> <FaRegCopy /> </button>
        </div>
...
const RentsDetailsBanner: React.FC<RentsDetailsBannerProps> = ({ villa }) => {
  ...
  const currentProperty = useSelector((state: any) => state.propertyBooking?.currentProperty);

  const effectiveVilla = villa ?? currentProperty ?? null;

  const propertyId = effectiveVilla?.id ?? 42;
  const title = effectiveVilla?.title ?? effectiveVilla?.name ?? "Seaclusion – Barbados' Platinum Coast";
  ...
  const previewImage =
    (Array.isArray(effectiveVilla?.media_images) && effectiveVilla.media_images[0]?.image) ||
    LOCAL_PREVIEW;
  ...
  const isRentType = listingType === "rent" || listingType === "rental" || listingType === "rentals";
    ...
    <div className="relative w-full h-[700px]">
      ...
      <div className="relative z-10 flex flex-col items-center justify-start h-full text-white p-4">
        <div className="text-center lg:mt-60 md:mt-56 mt-56">
          <h1 className="text-2xl md:text-4xl font-semibold drop-shadow-lg mb-2 leading-snug">{title}</h1>
          ...
          <div className="flex justify-around items-center text-center border-b pb-4 mb-4 flex-wrap gap-4">
            ...
            {isRentType && (
              <div className="flex flex-col items-center">
                <img src="https://res.cloudinary.com/dqkczdjjs/image/upload/v1760830630/user-fill_gkf8xf.png" alt="" />
                <span className="text-sm mt-1">{guestsCount} {pluralize(guestsCount, "Guest")}</span>
              </div>
            )}
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="text-sm mt-1">{bedsCount} {pluralize(bedsCount, "Bed")}</span>
            </div>

            {/* Baths */}
            <div className="flex flex-col items-center">
              <img src="https://res.cloudinary.com/dqkczdjjs/image/upload/v1760827484/Frame_4_zsqcrj.png" alt="" />
              <span className="text-sm mt-1">{bathsCount} {pluralize(bathsCount, "Bath")}</span>
            </div>

            {/* Pool */}
            <div className="flex flex-col items-center">
              <img src="https://res.cloudinary.com/dqkczdjjs/image/upload/v1760827483/Frame_5_cyajjb.png" alt="" />
              <span className="text-sm mt-1">{poolCount} {pluralize(poolCount, "Pool")}</span>
            </div>
          </div>

          <div className="flex flex-col items-center mb-4">
            <p className="text-lg font-medium text-green-700">
              {effectiveVilla?.price_display ? `From USD$${effectiveVilla.price_display}/night` : "From USD$850,000.00/night"}
            </p>
          </div>

          <div className="flex flex-col sm:flex-row gap-3">
            <button onClick={() => setIsShareModalOpen(true)} className="flex-1 bg-teal-600 text-white py-3 rounded">Share</button>
            ...
            {isRentType ? (
              <button onClick={() => {
                ...
                setIsModalOpen(true);
              }} className="flex-1 bg-teal-600 text-white py-3 rounded">Book Now</button>
            ) : null}
          </div>

          {feedbackMsg && <div className="mt-3 text-center text-sm text-green-700 font-medium">{feedbackMsg}</div>}
        </div>
      </div>
