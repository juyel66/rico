...
export default function UserManagement() {
  ...
  const getCurrentUserEmail = () => {
    try {
      const raw = localStorage.getItem("auth_user") || localStorage.getItem("user") || localStorage.getItem("current_user");
      if (raw) {
        try {
  ...
  const sortUsers = (list) => {
    ...
    const idx = copy.findIndex((u) => u && u.email && u.email.toLowerCase() === lower);
    if (idx > -1) {
  ...
  function startEdit(user) {
    const isCurrentUser = currentUserEmail && user.email && currentUserEmail.toLowerCase() === user.email.toLowerCase();
  ...
  async function saveEdit(id) {
      ...
      if (otherKeys.length > 0) {
        ...
        for (const k of otherKeys) {
          if ((edits[k] ?? "") !== (user[k] ?? "")) {
            ...
            toast.error("Cannot update email or role for your own account here.");
            return;
          }
        }
      }
    ...
    // optimistic update
    setUsers((prev) => sortUsers(prev.map((u) => (u.id === id ? { ...u, ...edits } : u))));
    ...
    try {
      ...
      if (updated) {
        ...
        setUsers((prev) => sortUsers(prev.map((u) => (u.id === id ? { ...u, ...updated } : u))));
      }
      ...
      if (oldData) setUsers((prev) => sortUsers(prev.map((u) => (u.id === id ? { ...u, ...oldData } : u))));
    } finally {
  ...
  async function handleDelete(id) {
    ...
    if (currentUserEmail && u.email && currentUserEmail.toLowerCase() === u.email.toLowerCase()) {
      Swal.fire({
        icon: "warning",
        title: "Action not allowed",
                      ...
                      {isEditing ? (
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            title={isCurrentUser ? "You can edit your name here only." : `Edit ${user.name}`}
                          >
                            Edit
                          </button>
                          <button
              ...
              <div key={user.id} className="bg-white border rounded-lg p-4 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex-shrink-0 text-right">
                    ...
                    {isEditing ? (
                      isCurrentUser ? (
                        <select value={user.role} disabled className="mt-1 px-2 py-1 border rounded-md text-sm bg-gray-50 cursor-not-allowed text-gray-500">
                          <option>{pretty(user.role)}</option>
                        </select>
                      ) : (
                        <select
