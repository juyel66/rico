...
const CreatePropertyRentals = () => {
  ...
  const buildStaffArray = (rows) => rows
    .filter(r => (r.name && r.name.trim()) || (r.details && r.details.trim()))
    .map(r => ({ name: r.name?.trim() || '', details: r.details?.trim() || '' }));
  ...
  return (
    <div className="p-6 md:p-10 bg-gray-50 min-h-screen w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-full mx-auto space-y-6">
        ...
        {/* Rules */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Rules (one per field)</label>
          <div className="space-y-3">
            {rules.map((v, i) => (
              <div key={i} className="flex gap-2 items-center">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => addArrayItem(setRules, rules, rulesRefs)} className="px-3 py-2 bg-teal-600 text-white rounded-lg">Add</button>
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-12 sm:col-span-6">
            <label className="block text-sm font-medium text-gray-700 mb-1">Check-in (HH:MM)</label>
            <input
              name="check_in"
              value={checkIn}
            ...
            />
            {errors.check_in && <p className="text-sm text-red-600 mt-1">{errors.check_in.message}</p>}
            <p className="text-xs text-gray-400 mt-1">Format: HH:MM (24-hour). Leave blank if not applicable.</p>
          </div>

          <div className="col-span-12 sm:col-span-6">
            <label className="block text-sm font-medium text-gray-700 mb-1">Check-out (HH:MM)</label>
            <input
              name="check_out"
              value={checkOut}
            ...
            />
            {errors.check_out && <p className="text-sm text-red-600 mt-1">{errors.check_out.message}</p>}
            <p className="text-xs text-gray-400 mt-1">Format: HH:MM (24-hour). Leave blank if not applicable.</p>
          </div>
        </div>
        ...
        {/* Staff rows */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Staff (add rows: name + details)</label>
          <div className="space-y-2">
            {staffRows.map((r, idx) => (
              <div key={idx} className="flex gap-2 items-start">
                ...
                <input
                  value={r.details}
                  onChange={e => updateStaffRow(idx, 'details', e.target.value)}
                  placeholder="Details (role, phone)"
                  className="flex-1 border rounded-lg p-2"
                />
                <div className="flex flex-col gap-2">
                  <button type="button" onClick={addStaffRow} className="px-3 py-1 bg-teal-600 text-white rounded">Add</button>
                  <button type="button" onClick={() => removeStaffRow(idx)} className="px-2 py-1 bg-red-100 text-red-600 rounded">x</button>
                </div>
              </div>
            ))}
          </div>
